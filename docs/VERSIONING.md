...
| `v0.3.0`, `v0.3.1`, `v0.3.2`, `v0.3.3`, `v0.3.4`, `v0.3.5`, `v0.3.6`, `v0.3.7`, ... | Immutable annotated release tags. |
...
remains immutable. The current public release is `v0.3.6`; the expanded-FK
verification and equivalent-rule correction release currently prepared is
`v0.3.7`.
...
| `Dockerfile` | default `APP_VERSION` build argument |
| `src/build-info.ts` | fallback `PRODUCT_VERSION` |
| `test-public/build-info.test.ts` | verifies package, Docker and runtime versions remain identical |
...
git commit -m "fix: resolve expanded GLPI relations during verification"
git add Dockerfile README.md docs package.json package-lock.json src/build-info.ts test-public/build-info.test.ts
git commit -m "chore(release): prepare version 0.3.7"
git tag -a v0.3.7 -m "Release glpi-mcp-docker v0.3.7"
...
- `0.3.7`, `0.3` and a commit-SHA tag after a push of `v0.3.7`.
